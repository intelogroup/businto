...
export const emailTemplates = {
  ...
  quotesBulkReceived: (data: {
    userName: string;
    requestId: string;
    quotes: { operatorName: string; price: number; vehicleType: string; note?: string }[];
    appBaseUrl?: string;
  }) => {
    const viewLink = `${getAppBaseUrl(data.appBaseUrl)}/trips/${data.requestId}`;
    const quoteRows = data.quotes.map((q) => `
      <tr style="border-bottom: 1px solid #e5e7eb;">
        <td style="padding: 12px 16px; font-weight: 600; color: #111827;">${q.operatorName}</td>
        <td style="padding: 12px 16px; font-size: 18px; font-weight: 700; color: #16a34a;">$${q.price.toFixed(2)}</td>
        <td style="padding: 12px 16px; color: #6b7280; text-transform: capitalize;">${q.vehicleType.replace(/_/g, ' ')}</td>
        <td style="padding: 12px 16px; color: #6b7280; font-size: 13px;">${q.note || '—'}</td>
      </tr>
    `).join('');
    return {
      subject: `${data.quotes.length} quotes ready for your transport request`,
      html: `
      <!DOCTYPE html>
      <html>
        <head>
          <style>
            ...
            .header { background: linear-gradient(135deg, #059669, #10b981); color: white; padding: 30px; border-radius: 12px 12px 0 0; text-align: center; }
            .content { background: #fff; padding: 30px; border: 1px solid #e5e7eb; border-top: 0; border-radius: 0 0 12px 12px; }
            .button { display: inline-block; background: #16a34a; color: white; padding: 12px 28px; border-radius: 8px; text-decoration: none; font-weight: 600; margin-top: 20px; }
            .footer { text-align: center; color: #9ca3af; font-size: 12px; margin-top: 30px; }
            table { width: 100%; border-collapse: collapse; margin: 20px 0; border-radius: 8px; overflow: hidden; border: 1px solid #e5e7eb; }
            th { background: #f9fafb; padding: 10px 16px; text-align: left; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: #6b7280; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin: 0; font-size: 24px;">${data.quotes.length} Quotes Ready!</h1>
              <p style="margin: 8px 0 0; opacity: 0.9;">Compare and accept the best offer</p>
            </div>
            <div class="content">
              <p>Hi ${data.userName},</p>
              <p>We've received <strong>${data.quotes.length} quote${data.quotes.length > 1 ? 's' : ''}</strong> for your transport request. Compare them below and accept the one that works best for you.</p>
              <table>
                <thead>
                  <tr>
                    <th>Operator</th>
              ...
              <p style="color: #6b7280; font-size: 14px;">Log in to your dashboard to accept a quote. Quotes expire in 2 hours.</p>
              <div style="text-align: center;">
                <a href="${viewLink}" class="button">Compare &amp; Accept Quotes</a>
              </div>
              <div class="footer">
                <p>&copy; 2026 Businto. All rights reserved.</p>
              </div>
    ...
    `,
      text: `Hi ${data.userName}, you have ${data.quotes.length} quotes ready. Compare and accept at: ${viewLink}`,
    };
  },
};
